...
class Sam3TrackerPredictor(Sam3TrackerBase):
    ...
    def load_first_frame(self, img):
        ...
        inference_state = self._init_state(
        ...
        inference_state["images"] = [
            img
        ]  # [img]是一个列表，包含一个元素，即第一帧的图像
        inference_state["num_frames"] = len(inference_state["images"])  # 1
        inference_state["video_height"] = height  # 360
        inference_state["video_width"] = width  # 800
        inference_state["frame_idx"] = 0  # 初始化frame_idx
        self._get_image_feature(
            inference_state=inference_state, frame_idx=0, batch_size=1
        )  # * 缓存第一帧的features
        return inference_state
    ...
    def _manage_memory_obj(self, inference_state, frame_idx, current_out):
        output_dict = inference_state["output_dict"]
    ...
    def track(
        ...
        # 从inference_state中获取并更新frame_idx
        frame_idx = inference_state.get("frame_idx", 0) + 1
        ...
        # output_dict[storage_key][frame_idx] = current_out
        self._manage_memory_obj(inference_state, frame_idx, current_out)

        _, video_res_masks = self._get_orig_video_res_output(
            inference_state, pred_masks_gpu
        )
